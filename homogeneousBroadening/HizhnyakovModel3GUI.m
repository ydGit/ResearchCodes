function varargout = HizhnyakovModel3GUI(varargin)
% HIZHNYAKOVMODEL3GUI MATLAB code for HizhnyakovModel3GUI.fig
%      HIZHNYAKOVMODEL3GUI, by itself, creates a new HIZHNYAKOVMODEL3GUI or raises the existing
%      singleton*.
%
%      H = HIZHNYAKOVMODEL3GUI returns the handle to a new HIZHNYAKOVMODEL3GUI or the handle to
%      the existing singleton*.
%
%      HIZHNYAKOVMODEL3GUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in HIZHNYAKOVMODEL3GUI.M with the given input arguments.
%
%      HIZHNYAKOVMODEL3GUI('Property','Value',...) creates a new HIZHNYAKOVMODEL3GUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before HizhnyakovModel3GUI_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to HizhnyakovModel3GUI_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help HizhnyakovModel3GUI

% Last Modified by GUIDE v2.5 26-Dec-2013 12:01:18

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @HizhnyakovModel3GUI_OpeningFcn, ...
                   'gui_OutputFcn',  @HizhnyakovModel3GUI_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function [C1, w1, D1, G1, C2, w2, D2, G2, C3, w3, D3, G3, C4, w4, D4, G4, T, P] = updateAllParams(handles)
...
C1 = str2num( get(handles.editCoupling1, 'String') );
C2 = str2num( get(handles.editCoupling2, 'String') );
C3 = str2num( get(handles.editCoupling3, 'String') );
C4 = str2num( get(handles.editCoupling4, 'String') );
%===========================================================

function [freq, I] = calculateAll(handles)

[C1, w1, D1, G1, C2, w2, D2, G2, C3, w3, D3, G3, C4, w4, D4, G4, T, P] = updateAllParams(handles);
...
if (get(handles.cb1, 'Value') == 1)
    [freq, I] = HizhAll(C1, w1, D1, G1, T, P, Fs, tmax);    
    empty = false;    
end
if (get(handles.cb2, 'Value') == 1)
    [freq2, I2] = HizhAll(C2, w2, D2, G2, T, P, Fs, tmax);
    if empty
        freq = freq2;
        I = I2;
        empty = false;
...
if (get(handles.cb3, 'Value') == 1)
    [freq3, I3] = HizhAll(C3, w3, D3, G3, T, P, Fs, tmax);
    if empty
        freq = freq3;
        I = I3;
        empty = false;
...
if (get(handles.cb4, 'Value') == 1)
    [freq4, I4] = HizhAll(C4, w4, D4, G4, T, P, Fs, tmax);
    if empty
        freq = freq4;
        I = I4;
        empty = false;
...
function [f, I] = HizhAll(C, w, D, G, T, P, Fs, tmax)

C = cast(C, 'double'); % QLM mode coupling strength, a.u.
...
y = exp( -pi*k*t.*g0 + 1i*2*pi*t.*P0 + 2*pi*1i*t.*P*k+C*(2*pi*t.*1i*k*D*(nbar+1.0/2)-2*pi*t.*1i*k*D*(nbar+1)*alpha/(1+alpha)+(1i*D*(nbar+1)/(lambda*(1+alpha))*log(1+alpha-alpha*exp(-2*pi*t.*k*lambda)))) );
...
%overSample = 1;
T = handles.e11T(1:N);%linspace(handles.e11T(1), handles.e11T(end), overSample*length(handles.e11T) );
g = zeros(1, length(T));
j = 1;
for i = T
...
% --- Executes just before HizhnyakovModel3GUI is made visible.
function HizhnyakovModel3GUI_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to HizhnyakovModel3GUI (see VARARGIN)
...
%=======================================================================
%-----------QLM 1 Parameters-----------
handles.C1 = 1.0; % coupling strength
handles.W1 = 114.0;
handles.D1 = -15.0;
handles.G1 = 5.0;
set( handles.editCoupling1,'String', num2str(handles.C1, handles.formatSpec) );
...
%-----------QLM 2 Parameters-----------
handles.C2 = 1.0; % coupling strength
handles.W2 = 114.0;
handles.D2 = -15.0;
handles.G2 = 5.0;
set( handles.editCoupling2,'String', num2str(handles.C2, handles.formatSpec) );
...
%-----------QLM 3 Parameters-----------
handles.C3 = 1.0; % coupling strength
handles.W3 = 114.0;
handles.D3 = -15.0;
handles.G3 = 5.0;
set( handles.editCoupling3,'String', num2str(handles.C3, handles.formatSpec) );
...
%-----------QLM 4 Parameters-----------
handles.C4 = 1.0; % coupling strength
handles.W4 = 114.0;
handles.D4 = -15.0;
handles.G4 = 5.0;
set( handles.editCoupling4,'String', num2str(handles.C4, handles.formatSpec) );
...
%======================================================================
% which modes are enabled by default? Checkboxes' initial states
...
plotAll(handles);
% Choose default command line output for HizhnyakovModel3GUI
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes HizhnyakovModel3GUI wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = HizhnyakovModel3GUI_OutputFcn(hObject, eventdata, handles) 
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
N = 15; % Number of experimental data points to calculate
...
[freqAllT, IAllT] = calculateAll(handles);
% plot All-T Model and Experiment
...
function editCoupling4_Callback(hObject, eventdata, handles)
% hObject    handle to editCoupling4 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
plotAll(handles);
% Hints: get(hObject,'String') returns contents of editCoupling4 as text
%        str2double(get(hObject,'String')) returns contents of editCoupling4 as a double


% --- Executes during object creation, after setting all properties.
function editCoupling4_CreateFcn(hObject, eventdata, handles)
% hObject    handle to editCoupling4 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function editCoupling3_Callback(hObject, eventdata, handles)
% hObject    handle to editCoupling3 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
plotAll(handles);
% Hints: get(hObject,'String') returns contents of editCoupling3 as text
%        str2double(get(hObject,'String')) returns contents of editCoupling3 as a double


% --- Executes during object creation, after setting all properties.
function editCoupling3_CreateFcn(hObject, eventdata, handles)
% hObject    handle to editCoupling3 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function editCoupling2_Callback(hObject, eventdata, handles)
% hObject    handle to editCoupling2 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
plotAll(handles);
% Hints: get(hObject,'String') returns contents of editCoupling2 as text
%        str2double(get(hObject,'String')) returns contents of editCoupling2 as a double


% --- Executes during object creation, after setting all properties.
function editCoupling2_CreateFcn(hObject, eventdata, handles)
% hObject    handle to editCoupling2 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function editCoupling1_Callback(hObject, eventdata, handles)
% hObject    handle to editCoupling1 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
plotAll(handles);
% Hints: get(hObject,'String') returns contents of editCoupling1 as text
%        str2double(get(hObject,'String')) returns contents of editCoupling1 as a double


% --- Executes during object creation, after setting all properties.
function editCoupling1_CreateFcn(hObject, eventdata, handles)
% hObject    handle to editCoupling1 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
